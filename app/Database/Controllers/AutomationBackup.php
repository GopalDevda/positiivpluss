...
// counting automotion
$All_automation = $sensor->where('supplier_id',$u_id)->where('status',1)->findAll();
$data['all_automation'] = count($All_automation);

$connect_data = $sensor->where('supplier_id',$u_id)->where('status',1)->where('current_status',3)->findAll();
$data['connect_data'] = count($connect_data);

$process_data = $sensor->where('supplier_id',$u_id)->where('status',1)->where('current_status',2)->findAll();
$data['processing_data'] = count($process_data);

$not_connect_data = $sensor->where('supplier_id',$u_id)->where('status',1)->where('current_status',1)->findAll();
$data['not_connect'] = count($not_connect_data);
...
// print_r($filter);
 ...
 if($filter){
   ...
   $item = $sensor->join('supplier_assessment', 'sensors.subboundary_id=supplier_assessment.id')->like("supplier_assessment.cp_name", $filter)->where("sensors.owner_id = $o_id or sensors.supplier_id = $u_id")->where("sensors.status", 1)->findAll();
 
 }else
 {
 $item = $db->query("SELECT * FROM sensors where (owner_id = $o_id or supplier_id = $u_id) and status=1 limit " . $_POST["start"] . ", " .$_POST["limit"])->getResultArray();
     // $item = $db->query("SELECT * FROM sensors where (owner_id = $o_id or supplier_id = $u_id) and status=1 ")->getResultArray();
}    // $item = $db->query("SELECT * FROM sensors where (owner_id = $o_id or supplier_id = $u_id) and status=1 ")->getResultArray();
...
$data= '';
     ...
     $electricity_amount_id = $Data_electricity->where('status',1)->where('electricity_id',$value['id'])->findAll();
...
// $month_id = $valuefff['monthly_name'];
...
$electricity_amount = $Data_electricity->where('status',1)->where('electricity_id',$value['id'])->where('MONTH(bill_date)',$jdf)->findAll();
      ...
      <div class="row">
        <div class="col-sm-12">
          <div id="accordionWrapa1" role="tablist" aria-multiselectable="true">
            <div class="card less-margin">
             
              <div class="card-body p-1 pb-0  ">
                
                <div class="row">
                  <div class="col-md-2">
                    <p class="pb-0 mb-0 fs-smaller">'; 
                    if($value['current_status'] == '3'){
                     foreach($electricity_amount as $bb){ 
                       if($bb['electricity_id'] == $value['id'])
                       {
                      $date = time();
                      ...
                      $date1 = time();
                      $middle1 = strtotime($value['current_time_date']); 
                      $data.='<span>'.date('d-M-Y h:i:s:A', $middle1).'</span>';

                      }
                     $data.='</p>
                    <h5 class="fw-bolder"></h5>
                    <img src="'.base_url('public/icon/electricity.png').'" width="50px">
                  </div>';
                 
                   $data.='<div class="col-md-3 pt-2">
                    <h6 class="fw-bolder">Site Name</h6>
                    <p class="fs mb-0" data-id="'.$value['id'].'" onclick="collapase(this)"  class="accordion-button"
                      data-bs-toggle="collapse"
                      role="button"
                      data-bs-target="#faq-payment-1-site'.$value['id'].'"
                      aria-expanded="true"
                      aria-controls="faq-payment-1'.$value['id'].'">';
                      
                      if(!$value['sub_site'] == 0){
                       foreach($sub_boundary_item as $bb){ 
                      foreach($sub_site_view as $sub_site_id){ 
                       if($bb['id'] == $value['subboundary_id']){
                      if($sub_site_id['id'] == $value['sub_site']){
                      $data.='<span>'.$bb['cp_name'].'</span>';    $data.='<span>'."<b>|</b>".'</span>';
                     $data.='<span>'.$sub_site_id['sub_site_name'].'</span>';
                      
                      $i = $sub_site_id['sub_site_address']."<b>|</b>".$bb['cp_address'];
                      
                        $data.='<p class="fs" id="rr'.$value['id'].'">'.substr_replace($i, "...", 29).'</p>';
                       $data.='<input  type="hidden"  id="vali'.$value['id'].'">';
                     ...
                     $data.='<span>'.$bb['cp_name'].'</span>';   $data.='<span>'." ".'</span>';
                    ...
                    $data.='</p>
                    <div
                      id="faq-payment-1-site'.$value['id'].'"
                      class="collapse"
                      aria-labelledby="paymentTwo"
                      data-bs-parent="#faq-payment-qna"
                      >
                      <div class="fs accordion-body p-0 pb-1">';
                      if(!$value['sub_site'] == 0){
                       foreach($sub_boundary_item as $bb){ 
                      foreach($sub_site_view as $sub_site_id){ 
                       if($bb['id'] == $value['subboundary_id']){
                      ...
                      $i = $sub_site_id['sub_site_address']." <b>|</b> ".$bb['cp_address'];
                        ...
                        $data.='<p class="fs" id="rr'.$value['id'].'">'.$i.'</p>';
                  ...
                  <div class="col-md-2 pt-2">
                    ...
                    foreach($electricity_board as $ele_id){
                      if($ele_id['id'] == $value['provider_type']){
                  ...
                  <div class="col-md-1 pt-2">
                    ...
                    $data.='<form class="form" method="post" action="" enctype="multipart/form-data">
                      <input type="hidden" name="status_id" value="'.$value['id'].'">
                      <span class="firstconnect badge badge-light-danger" data-id="'.$value['id'].'">Connect</span>
                    </form>';
                }
                  ...
                  if($value['current_status'] == '3'){
                     ...
                     $data.='<span class="rohit badge badge-light-success"  data-id="'.$value['id'].'">Connected</span>';
                   ...
                   $data.='</div>';
                     ...
                     <div class="dropdown">
                        ...
                        <div class="dropdown-menu dropdown-menu-end">     
                          <a class="dropdown-item d-flex align-items-center" data-id="'.$value['id'].'"
                            data-boundary="'.$value['boundary_id'].'"
                            data-boardtype="'.$value['board_type'].'"
                            data-subboundary="'.$value['subboundary_id'].'"
                            data-subsite="'.$value['sub_site'].'"
                            data-provider="'.$value['provider_type'].'"
                            data-username="'.$value['username'].'"
                            data-password="'.$value['password'].'"
                            data-knno="'.$value['kn_no'].'"
                            data-consumerno="'.$value['consumer_no'].'"
                            data-accountno="'.$value['account_no'].'"
                            data-mobileno="'.$value['mobile_no'].'"
                            data-serviceno="'.$value['service_no'].'"
                            data-cano="'.$value['ca_no'].'"
                            data-holdername="'.$value['conHolderName'].'"
                            data-pdf="'.$value['consent_form_file'].'"
                            onclick="editmodel(this);">
                            <i data-feather="edit-2" class="me-50"></i><span>Update</span>
                          </a>
                       
                      
                          <a class="dropdown-item d-flex align-items-center" data-id="'.$value['id'].'" onclick="electricity_delete('.$value['id'].')" href="#">
...
// foreach($Energy as $energy_data)
// {

// foreach($Energy_model_data as $Energy_model_lop){ 

//     $site = json_decode($energy_data['site_id']);
//     $id = json_decode($energy_data['id']);
//     $idd = json_decode($Energy_model_lop['id']);
//     $supplier_id = json_decode($energy_data['supplier_id']);

//     foreach ($site as $key => $value) 
//     {

// $sensor = $Sensor_Model->select('*')->join('data_electricity','data_electricity.electricity_id=sensors.id')->where('sensors.subboundary_id',$value)->where('sensors.current_status',3)->where('sensors.status',1)->where('data_electricity.status',1)->findAll();


//      foreach($sensor as $sensor_data)
//      {  

//        if($sensor_data['subboundary_id'] == $value){
        

//         $consumeUnit += $sensor_data['consume_unit'];
        
//         $dataEnergy = ['value' => $consumeUnit];

//         $Energy_model->update($id,$dataEnergy);
//         $Energy_model_data->update($idd,$dataEnergy);
