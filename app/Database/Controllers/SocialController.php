...
$data['social_indicator'] = 
$db->query("SELECT * FROM `indicator` WHERE indicator_category_id = 9")->getResultArray();
         
            echo view('brand/social_view',$data);
...
public function social($id){
  ...
  $rs = check_session();
       ...
       $success = 0;
        ...
        $data['social_indicator']= $db->query("SELECT * FROM `indicator` WHERE indicator_category_id = 9 and id='".$id."'")->getResultArray();
...
$query = $db->query("select * from indicator where status=1 and id= '".$jid."'order by id");
    ...
    //   echo "\n";
    //   die();
    echo view('brand/social_indicator_view',$data);
