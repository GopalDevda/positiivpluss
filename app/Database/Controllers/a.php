...
//echo "select s.sdg_id,indicator_id from industry_sdg as s join indicator_sdg as b on s.sdg_id=b.sdg_id where (industry_id='".$supplier_info['industry_id']."' or industry_id=0) group by sdg_id";

        $query = $db->query("select s.sdg_id,indicator_id from industry_sdg as s join indicator_sdg as b on s.sdg_id=b.sdg_id where (industry_id='".$supplier_info['industry_id']."' or industry_id=0) group by sdg_id");
        $sdg = $query->getResultArray();
                       $total_question=0;
                        $total_attempt = 0;
                        $array=array();
        ...
        if(!empty($sdg)){
           foreach($sdg as $d){
                    ...
                    $query = $db->query("select * from indicator where status=1 and id='".$d['indicator_id']."' " );
                    ...
                    if(!empty($indicator)){
                      foreach($indicator as $i){
                            ...
                            $cat = $query->getResultArray();
                            $query = $db->query("select * from sdg_assessment_question where indicator_id='".$i['id']."' and sdg_id='".$d['sdg_id']."' and status=1 and (industry_id='".$supplier_info['industry_id']."' or industry_id=0) order by id ");
                ...
                $array[] = array('id'=>$i['id'],'indicator_name'=>$sdg_name[0]['sdg_name'],'image'=>$i['image'],'indicator_category_name'=>$cat[0]['indicator_category_name'],'question'=>$question_array,'question_attempt'=>$question_attempt[0]['tot'],'from_date'=>$from_date,'to_date'=>$to_date,'is_submit'=>$is_submit);
                            }
                // $query = $db->query("select count(distinct( s.base_assessment_question_id))as tot from supplier_base_assessment as s join sdg_assessment_question as b on s.base_assessment_question_id=b.id where b.indicator_id='".$i['id']."'  and s.supplier_id='".$supplier_info['supplier_id']."' ");
                // $question_attempt = $query->getResultArray();
                // $total_attempt = $total_attempt+$question_attempt[0]['tot'];
                // $total_question = $total_question+count($question_array);
                // $array[] = array('id'=>$i['id'],'indicator_name'=>$sdg_name[0]['sdg_name'],'image'=>$i['image'],'indicator_category_name'=>$cat[0]['indicator_category_name'],'question'=>$question_array,'question_attempt'=>$question_attempt[0]['tot'],'from_date'=>$from_date,'to_date'=>$to_date,'is_submit'=>$is_submit);
    ...
    public function submitSdgAssessmentQuestion($assessment_id,$qid,$answer,$remark,$indicator,$choice,$answer_insert_id){
       ...
       if($choice=='Multi'){
            ...
            $query = $db->query("select * from sdg_assessment_answer where status=1 and id='".$answer."' order by id asc");
       ...
       $query = $db->query("select count('id') tot from sdg_assessment_question where status=1 and indicator_id='".$indicator."' order by id asc");
       ...
       $query = $db->query("select count(distinct(s.base_assessment_question_id)) as tot from supplier_base_assessment as s join sdg_assessment_question as b on s.base_assessment_question_id=b.id where b.indicator_id='".$indicator."'  and s.supplier_id='".$supplier_info['supplier_id']."'  ");
  ...
  function submitAdvanceAssessment(){
    ...
    public function verifyAdvanceAssessment() {
        ...
        if($supplier_info['supplier_id']) {
            ...
            $supplier_base_assessment = $db->query("select sba.base_assessment_question_id,baq.id as qid from supplier_base_assessment as sba join sdg_assessment_question as baq on sba.base_assessment_question_id=baq.id where sba.supplier_assessment_id=".$supplier_assessment->id." and baq.is_document_needed=1")->getResultArray();
    ...
    public function checkQuestionCompletion() {
        ...
        // $query = $db->query("SELECT s.id,s.indicator_name,s.indicator_category_id,s.image FROM `indicator` as s join sdg_assessment_question as b on s.id=b.indicator_id where s.status=1 and b.status=1 and b.industry_id='".$supplier_info['industry_id']."' group by b.indicator_id order by s.id" );
        // $indicator = $query->getResultArray();
        $query = $db->query("select indicator_id,s.sdg_id from industry_sdg as s join indicator_sdg as b on s.sdg_id=b.sdg_id where (industry_id='".$supplier_info['industry_id']."' or industry_id=0) group by s.sdg_id");
        $sdg = $query->getResultArray();
        $total_question=0;
        $total_attempt = 0;
        if(!empty($sdg)){
        ...
        if(!empty($indicator)) {
            foreach($indicator as $i) {
                $query = $db->query("select count(id) as cnt from sdg_assessment_question where indicator_id='".$i['id']."' and status=1 and industry_id='".$supplier_info['industry_id']."' and sdg_id='".$d['sdg_id']."' order by id ");
                $question = $query->getResultArray();
                if($question) {
                    $total_question = $total_question+$question[0]['cnt'];          
                }
                $query = $db->query("select count(distinct( s.base_assessment_question_id))as tot from supplier_base_assessment as s join sdg_assessment_question as b on s.base_assessment_question_id=b.id where b.indicator_id='".$i['id']."' and b.sdg_id='".$d['sdg_id']."' and s.supplier_id='".$supplier_info['supplier_id']."' and s.supplier_assessment_id='".$assessment_id."' ");
                $question_attempt = $query->getResultArray();
                $total_attempt = $total_attempt+$question_attempt[0]['tot'];
            }
        }
