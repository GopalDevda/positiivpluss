        ...
        public function resendotp()
        {
              ...
              function submitAssessment(){
                ...
                public function document(){
                    ...
                    if(!empty($doc)){
                        foreach($doc as $r){
                            ...
                            $supplier_doc[]=array('id'=>$r['id'],'owneremail'=>$r['owneremail'],'titleu'=>$r['title'],'ownername'=>$r['ownername'],'ownerimg'=>$r['ownerimg'],'document_name'=>$r['document_name'],'approved_on'=>$r['approved_on'],'document_type'=>$type[0]['document_type_name'],'file_size'=>$r['file_size'],'image'=>$r['image'],'document_type_id'=>$r['document_type_id'],'date'=>$r['date'],'doc_connect_count' => $document_connect_count->cnt);
                        }
                    }
                ...
                public function addDocument(){
                    ...
                    if($action_id=='2'){
                        $newName="";
                        $file_size=0;
                        if($model->insert(['supplier_id'=>$supplier_info['supplier_id'],'document_sub_type_id'=>$document_sub_type_id,'image'=>$newName,'action_id'=>$action_id,'details'=>$description,'status'=>1,'document_type_id'=>$type_id,'document_name'=>$doc_name,'file_size'=>$file_size,'date'=>date('Y-m-d')])){
                    ...
                    $ava_ext = array("png", "jpg", "jpeg", "gif","pdf","doc","docx");
                        if(in_array($ext,$ava_ext)){
                            ...
                            if($file->move('public/uploads/supplier_document',$newName)){
                                if($model->insert(['supplier_id'=>$supplier_info['supplier_id'],'action_id'=>$action_id,'image'=>$newName,'status'=>1,'document_type_id'=>$type_id,'document_name'=>$doc_name,'file_size'=>$file_size,'date'=>date('Y-m-d')])){
                                    $session->setFlashdata('success','Document has been successfully upload');
                                }else{
                                    $session->setFlashdata('error','Document not upload');
                                }
            ...
            $data.='        <div class="card-header header-elements-inline" id="colorId0">
                ...
                public function updateselfDocApproval(){
                    ...
                    $insert= $db->query("update supplier_document set approved_on='".$approved_on."' , ownername='".$supplier_info['supplier_name']."' ,ownerimg='2.jpg' , owneremail='".$supplier_details[0]['email']."' , self_approved=1 where id='".$doc_id."'and supplier_id='".$supplier_info['supplier_id']." '");
