...
//      $crudModel = new Supplier_assessment();
...
// die();
     ...
     $query = $db->query("select sa.*,ft.type_name,u.unit_name,c.name as country_name,k.name as state_name from supplier_assessment as sa left join footprint_type as ft on sa.cp_type_id=ft.id left join countries as c on sa.country_id=c.id left join states as k on sa.state_id=k.id left join unit as u on u.id=sa.unit_id where sa.supplier_id='".$supplier_info['supplier_id']."'and assessment_id=12  order by sa.id asc")->getResultArray();
...
$i=1;
      ...
      foreach($query as $r) {
       $id =  $r['id'];
         $sub_site = $model->where('supplier_id',$supplier_id)->where('status',1)->where('site_id',$id)->findAll();
...
if($addresproof){
   ...
   $validated = $this->validate([
    ...
    public function report_page(){
        ...
        $query = $db->query("select standard_name from standard where status=1");
...
//         echo 'success';
    ...
    public function productTransportAssessmentSubmit() {
        ...
        }elseif($factor2>0 || $factor3>0){$factor=$factor2;}elseif($factor4!==0 || $factor5!==0){$factor=$factor4;}
...
<script>
    ...
    $data['sub'] = $db->query("select * from supplier_assessment where  supplier_id=".$supplier_id."")->getResultArray();
    ...
    public function site_data_show()
    {
  $rs = check_session();
...
// print_r($response);
// die();
$l = json_encode($response);
print_r($l);
die();

    return $this->response->setJSON($l);    
...
public function sub_site_view($id) {
    ...
    $data['sub'] = $db->query("select * from supplier_assessment where  supplier_id=".$supplier_id."")->getResultArray();
    $data['subsite'] = $db->query("select * from supplier_assessment where  supplier_id=".$supplier_id." and id=".$id."")->getResultArray();
 ...
 // $data['sub_site'] = $db->query("SELECT * FROM sub_site WHERE status=1 and supplier_id='".$supplier_id."'  ")->getResultArray();
    ...
    $data['sub_site'] = $model->where(['site_id'=>$id,'status'=>'1','supplier_id'=>$supplier_id])->findAll();
    ...
    public function createCompany() {
      ...
      if($building_size == ""){
        ...
        $t = $db->query("insert into supplier_assessment(is_submit,assessment_id,supplier_id,lease_owned,pincode,city_id,owner_id,no_of_employee,cp_name,state_id,branch_code,building_size,unit_id,cp_type_id,cp_address,country_id,date_from,date_to) values(1,12,'".$supplier_info['supplier_id']."','".$lease_owned."','".$pincode."','".$city."','".$o_id."',".$total_employees.",'".$company_name."','".$state_id."',
            ...
            '".$branch_code."','".$building_size."','".$unit_id."',".$company_type.",'".$company_address."','".$country_id."','".date('Y-m-d')."','".date('Y-m-d')."')");
...
$data['n_per'] = $data['difableNo']/$total*100;
        ...
        $offices = json_decode($quickStart['offices']);
                 ...
                 if($location){ foreach($location as $key => $row){
                   if($row == 1)
                    {
                      if($offices[$key])
                  {
     ...
     $plan_total  = '1';
      ...
      $data['pla_per_in'] = $inter_plan/$plan_total*100;
       ...
       $employee_total = json_decode($quickStart['employee_total']);
            ...
            if($employee_perr){$emp_male_total = 0; foreach($employee_perr as $key => $row){
                ...
                if($employee_perr){$wor_male_total = 0; foreach($employee_perr as $key => $row){
               ...
               // die();
                ...
                if ($employee_perr) {$emp_femal_total=0; foreach($employee_perr as $key => $row){
                if($employee_wor[$key] == 1)
                {
                ...
                if ($employee_perr) {$wor_femal_total=0; foreach($employee_perr as $key => $row){
                if($employee_wor[$key] == 2)
                {
                if($employee_gen[$key] == 'female')
                {
                ...
                if ($employee_perr) {$emp_other_total=0; foreach($employee_perr as $key => $row){
                if($employee_wor[$key] == 1)
                {
                ...
                if ($employee_perr) {$wor_other_total=0; foreach($employee_perr as $key => $row){
                if($employee_wor[$key] == 2)
                {
                if($employee_gen[$key] == 'other')
                {
                ...
                // print_r($data['emp_male_total']);echo',';
                // print_r($data['wor_male_total']);echo',';
                // print_r($data['emp_femal_total']);echo',';
                // print_r($data['wor_femal_total']);echo',';
                // print_r($data['emp_other_total']);echo',';
                // print_r($data['wor_other_total']);echo',';
                // die();
       ...
       $women_total = json_decode($quickStart['women_total']);
                ...
                // KEy Total male
                if ($women_bord) { foreach($women_bord as $key => $row){
                if($row == '2')
                {
                if($women_gen[$key] == 'male')
                {
                ...
                else
                {
                 ...
                 // die();
                  // female total bod
                if ($women_bord) { foreach($women_bord as $key => $row1){
                if($row1 == '1')
                {
               ...
               // female total key
                ...
                else
                {
                  ...
                  // other total BOD
                if ($women_bord) { foreach($women_bord as $key4 => $row4){
                if($row4 == '1')
                {
