...
<div class="app-content content">
  <input type="hidden" name="idss" id="idss" value="1">
            ...
            <div class="row">
              ...
              <div class="col-md-6 col-12">
                <div class="mb-1">
                  ...
                  <select class="form-control js-example-basic-single select2 siteName" id="subboundary_id" name="sub_boundary" required>
              ...
              <div class="col-md-6 col-12" id="sub_site_id_show" style="display:none;">
                <div class="mb-1">
                  ...
                  <select class="form-control select2 sub_site_data" id="sub_site_id" name="sub_site">
    ...
    <div class="content-header row">
      <div class="content-header-left col-md-8 col-12 mb-2">
        <div class="row breadcrumbs-top">
          ...
          <div class="col-md-8"><input type="text" name="subbuid" class="form-control" id="subbuid" onkeyup="get_sensor_by_search($(this).val())" placeholder="Search site name to filter"></div>
          <!-- <input type="hidden" name="" id="run"> -->
        </div>
      </div>
      <!-- -->
    </div>
...
<!-- barchart script-->
<!-- Site Already exsite not create new site -->
<script>
  $(document).ready(function() {
    $('.siteName').on("change", function() {
      var siteName = $(this).val();
      var siteSubName = $("#sub_site_id").val();
      // alert(siteSubName);
      // alert(siteName);
      $.ajax({
        url: '<?php echo base_url() ?>/Automotion/checkSiteExists',
        type: 'get',
        data: {
          subboundary_id: siteName,
          sub_site: siteSubName,
        },
        // dataType: 'json',
        success: function(response) {
          ...
          if (response.true) {
            // Show error message
            $('#nameError').text("Already Automation Created.");
          } else {
            $('#nameError').hide();
          }
        },
...
<script>
  $(document).ready(function() {
    $('.sub_site_data').on("change", function() {
      var siteSubName = $(this).val();
      var siteName = $("#subboundary_id").val();
      // alert(siteSubName);
      // alert(siteName);
      $.ajax({
        url: '<?php echo base_url() ?>/Automotion/checkSiteExists',
...
<script>
  ...
  //   $('#idss').val(1);
  //   $.ajax({
  //     url: "<?= base_url() ?>/Automotion/sensor_data_view/" + val,
  //     type: "post",
  //     data: {
  //       "idss": 1
  //     },
...
<!-- <script type="text/javascript">
  var id_show = true; // If this is true than the sensordata fetch will show in #sensordatafetch-1 else sensordatafetch-2
  var limit = 50; //The number of records to display per request
  var start = 0; //The starting pointer of the data
  var run = true;
  // $('#subbuid').change(function() {
  //   console.log("working");
  //   load_country_data(limit, start);
  // });
  function get_sensor_by_search(data) {
    $('#run').val(1);
    // console.log("work");
    // delay(function(e) {
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        if ($('#run').val() == "1") {
          $('#run').val('0');
          $("#Sensordatafetch").html(data);
        } else $("#Sensordatafetch").append(data);
        let d = $("#Sensordatafetch").html();
        if (d == '') {
          $("#Sensordatafetch").html(`<h3 class="text-center mt-2">No Record Match</h3>`);
          run = false;
        } else if (data == '') {
          if (!(d.includes("End Of Records")))
            $("#Sensordatafetch").append(`<h3 class="text-center mt-2">End Of Records</h3>`);

          run = false;
        }
        // console.log((d.includes("End Of Records")));
  ...
  $(window).scroll(function() {
    if ($(window).scrollTop() + $(window).height() > $("#Sensordatafetch").height()) {
      //  action = 'active';
      console.log($('#run').val());
...
<script type="text/javascript">
  var limit = 25; //The number of records to display per request
  var start = 0; //The starting pointer of the data
  ...
  //  var searchData;
  $("#subbuid").keyup(function(e) {
    // e.preventDefault();
    setTimeout(function() {
      var searchData = $('#subbuid').val();
      ...
      if (searchData != "") {
        ...
        $.ajax({
          ...
          success: function(resSearch) {
            $("#Sensordatafetch").html("");
            // $("#Sensordatafetch").append(resSearch);
            $("#Sensordatafetch").append(resSearch);
            if (resSearch == '')
              $("#Sensordatafetch").html(`<h3 class="text-center mt-2">No Record Match</h3>`);
            // alert(resSearch);
        ...
        $.ajax({
          ...
          success: function(data) {
            if (data == '') {
              // alert(data);
              $("#sub_site_id_show").hide();
              // $('#nameError').text("Please Craete a Subsite.");
            } else {
