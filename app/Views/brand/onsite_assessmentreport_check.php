...
<div class="app-content content">
    ...
    <div class="content-wrapper container-xxl p-0" id="html-to-pdf">
            ...
            <div class="content-header-left col-md-12 col-12 mb-2">
                <div class="row breadcrumbs-top">
                    <div class="col-12">
                        ...
                        <?php if ($nextbtn == 0) { ?>
                            <a href='<?php echo base_url('Controlwork/assessment_view') ?>/<?= $id; ?>' class='btn btn-primary float-end mx-2'>Back</a>
                        <?php
                        } else { ?>
                            <a href='<?php echo base_url('Controlwork/assessment_view_supplier') ?>' class='btn btn-primary float-end mx-2'>Back</a>
                        <?php } ?>
        ...
        <div class="content-body">
            ...
            <table class="table">
                ...
                <tbody>
                    <tr style="background:#e9ecef;">
                        ...
                        <td><b></b> </td>
                        <td><b></b></td>
                        <td><b><?= $per = number_format($percentile1, 1) ?>%</b> </td>
                        <td><?php if ($per >= $row['failed_per']) { ?><button class="btn btn-success">Pass</button> <?php
                                                                                                                } else { ?><button class="btn btn-danger">Failed</button><?php
                                                                                                                                                                        } ?></td>
                    </tr>
                    <tr>
                        <th><?= $boundryname; ?></th>
                        <td><?php foreach ($control_assessment as $row) {
                                if ($row['boundary'] == 43) {
                                    ...
                                    foreach ($assessm1 as $dd1) {
                                        // print_r($row['sub_boundary']);
                                        if ($dd1['id'] == $row['sub_boundary']) {
                                            echo  $dd1['type_name'];
                                        }
                                    }
                            ?>
            ...
            <form action="<?= site_url('OnSite/assessment_report_insert') ?>" method="post">
                <?php $i = 0;
                ...
                foreach ($assessmentreport_data as $key => $show) {
                    if ($ind_id != $show['indicator_id']) {
                        echo "<div class='p-1 h3' style='background:#e9ecef;'>" . $indicator_model->where('id', $show['indicator_id'])->first()['indicator_category_name'] . "</div>";
                        $ind_id = $show['indicator_id'];
                    }
                    $countpage = count($show) / 5; ?>
                    <div class="container c1 pt-3">
                        <input type="hidden" name="ass_ids[]" value="<?= $show['id'] ?>">
                        <h5 class="text-dark">
                        ...
                        <div style="float: right;">
                            <?php if ($show['answer'] == json_encode('No')) { ?>
                                <div class="box2 pt-2" style="display: flex;">
                                    <div style="margin-top:0px!important;margin-right:30px">
                                        <label for="" class="form-label text-dark">Auditor Comment</label><br>
                                        <input type="text" class="form-control" name="input_text[]" value="<?= $show['onsite_comment'] ?>">
                                    </div>
                                    <div style="margin-top: 0px;margin-right: 70px">
                                        <label for="" class="form-label text-dark">Take Action</label><br>

                                        <input type="checkbox" style="height:24px;width:24px;" data-id="<?= $show['id'] ?>" class="checkboxes" <?= $show['onsite_checked'] ? 'checked' : '' ?>>
                                        <input type="hidden" name="checkboxes[]" id="checkb<?= $show['id'] ?>" value="<?= $show['onsite_checked'] ?>">
                                    </div>
                                    <?php if ($show['media']) { ?>
                                        <div style="text-align: center;margin-right: 1rem;">
                                            <i class="fa fa-eye" onclick="window.open('<?= base_url('public/uploads/ans_question/' . $show['media']) ?>','_blank')"></i>
                                            <label for="myButton" class="form-label text-warning">Attachment</label><br>
                                            <a class=" btn btn-warning" href=" <?= base_url('public/uploads/ans_question/' . $show['media']) ?>" download="<?= $show['media_name']; ?>"><?= $show['media_name']; ?> <i class="fa-solid fa-download"></i></a>
                                        </div>
                                    <?php
                                    } ?>
                                    <div style="text-align: center;">
                                        <label for="" class="form-label text-danger">Answer</label><br>
                                        <button class="btn btn-danger"><?= $show['answer']; ?></button>
                                    </div>
                                </div>
                            <?php } else if ($show['answer'] == json_encode('Yes')) { ?>
                                ...
                                <div class="box2 pt-2" style="display: flex;">
                                    ...
                                    <div style="text-align: center;">
                                        ...
                                        <button class="btn btn-success"><?= $show['answer']; ?></button>
                                    </div>
                                </div>
                            <?php } else { ?>
                                <div class="box2 pt-2" style="display: flex;">
                                    ...
                                    <?php if ($show['media']) { ?>
                                        <div style="text-align: center;margin-right: 1rem;">
                                            <i class="fa fa-eye" onclick="window.open('<?= base_url('public/uploads/ans_question') ?>/<?= $show['media']; ?>','_blank')"></i>
                                            <label for="myButton" class="form-label text-warning">Attachment</label><br>
                                            <a class=" btn btn-warning" href=" <?= base_url('public/uploads/ans_question') ?>/<?= $show['media']; ?>" download="<?= $show['media_name']; ?>"><?= $show['media_name']; ?>
                        ...
                        <div class="container">
                            <div class="box33" style="width:50%; ">
                                <h5>
                                    <?php if (!$show['comment'] == '') { ?>
                                        Comment:- <span><?= $show['comment'] ?></span></h5>
                            <?php
                                    } else { ?>
                                <h5>
                ...
                <textarea name="remark" class="form-control"><?= $assessmentreport_data[0]['remark']; ?></textarea>
                <div class="pt-1">
...
<script>
    $('.checkboxes').click(function() {
        if (this.checked) {
            $('#checkb' + $(this).data('id')).val(1);
        } else $('#checkb' + $(this).data('id')).va(0);
        // $(this).parent().find("input[name='checkboxes'").val(0);
    })
