...
use App\Models\Quantitative_Footprint_Model;
...
class Quantitative extends BaseController
{
    ...
    public function footprints()
    {
        ...
        $pending_notify = $db->query("SELECT * FROM `control_footprints` WHERE owner_id = $o_id and status=1 and complete=0")->getResultArray();


        foreach ($pending_notify as $value) {
            foreach ($supplier_compare as $compare) {
                if ((!$value['reminder_send']) && ($compare['id'] == $value['assigned_to'])) {
                    ...
                    if ($diff->format("%R%a") <= 0) {
                        $compare_supplier_name = $compare['supplier_name'];
                        $compare_image = $compare['image'];
                        $compare_email = $compare['email'];
                        $indicator_show = ($ghg_model->where('id', $value['ghg'])->first())['ghg_name'];
    ...
    public function start_footprint($main_id)
    {
        ...
        // $query = $db->query("SELECT ed.* FROM `employee_details` as ed where ed.status=1 ");
        // $data['employee_details'] = $query->getResultArray();
        $ghg_id = $footprint->where('id', $main_id)->where('status', 1)->first()['ghg'];
        $data['footprint_all'] = $footprint->where('ghg', $ghg_id)->where('status', 1)->findAll();
        ...
        $data['vehical_transportion'] = $a->where('status', '1')->where('footprint_id', '2')->findAll();
        ...
        $data['footprint_all'] = $db->query("SELECT id, factor_name FROM `factor` WHERE `ghg_id`= $main_id and status=1")->getResultArray();
        // print_r($data['vehical']);
        // die;
        $find = $supplier_model->where('id', $u_id)->first();
        ...
        $ghg_model = new GhgModel();
        $rs = $db->query("select * from ghg_factor where status=1 and ghg_id=$ghg_id")->getResultArray();
        if ($rs) {
            $factor_id = $db->query("select * from factor where id=" . $rs[0]['name'])->getRow()->factor_unit;
            foreach ($rs as $r) {
                $factor_unit_name = $factor_unit_id = '';
                $ghg = $ghg_model->where('id', $r['ghg_id'])->first();
                $factor = $db->query("select * from factor where id=" . $r['name'])->getRow();
                if ($factor) {
                    $factor_name = $factor->factor_name;
                    $factor_id = $factor->id;
                    $factor_unit_name = $db->query("SELECT id, unit_name FROM `unit` WHERE status=1 and id=$factor->factor_unit")->getResultArray()[0]['unit_name'];
                    $factor_unit_id = $factor->factor_unit;
                } else $factor_name = $r['name'];
                $list[] = [
                    "id" => $r["id"],
                    "factor_name" => $factor_name,
                    "factor_id" => $factor_id,
                    'unit_name' => $factor_unit_name,
                    'unit_id' => $factor_unit_id,
                ];
            }
        }
        ...
        $data['list'] = $list;
        $data['flight_detail']
            = $db->query("SELECT * FROM `flight_detail` WHERE status=1 GROUP BY flight_class")->getResultArray();
        // print_r($data['flight_detail']);
        // die;
    ...
    public function answerFootprint($main_id)
    {
        ...
        $footprint_value = $this->request->getVar('footprint_value');
        ...
        if (is_array($unit)) $unit = json_encode($unit);
        ...
        if ($form_type == 9) {
            $footprint = new Quantitative_Footprint_Model();
        } else {
            $footprint = new QuantitativeFootprintAnswerModel();
        }
        $vehical_no = $this->request->getVar('vehical_no');
        ...
        if ($vehical_no) {
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://api.invincibleocean.com/invincibleVehicleFastest',
    ...
    "vehicleNumber": "KA05LE7925"
        ...
        if ($transport_type) $datas['transport_type'] = $transport_type;
        if ($form_type == 3) {
            $result_type = 1;
            $data = [
                // 'completed_by' => $u_id,
            ...
            if ($footprint_value) $data['footprint_value'] = $footprint_value;
        }
        $data = array_merge($datas, $data);
        $emission = 0;
        if ($form_type == 1) {
            $country_name = $this->request->getVar('country_name');
            $country_id = $db->query("SELECT * FROM countries where name='$country_name'")->getResultArray()[0]['id'];
            $emission = $db->query("SELECT * FROM hotel_stay_detail where country_id=$country_id")->getResultArray()[0]['emission'];
            $land_api = 1;
        }
        $footprint_val = $footprint->where("status=1 and owner_id=$o_id and footprint_id=$main_id and transport_type=$transport_type")->first();
        if ($footprint_val) {
            $t_where = "transport_type=$transport_type";
        } else {
            $t_where = " transport_type=0";
        }
        $where = "status=1 and owner_id=$o_id and footprint_id=$main_id and $t_where";
        $footprint_answer = $footprint->where($where)->first();
        if ($footprint_answer) {
            $save = $footprint->update($footprint_answer['id'], $data);
        } else {
            $save = 1;
            if (!$footprint->where("status=1 and owner_id=$o_id and footprint_id=$main_id")->first())
        ...
        if ($save) {
            ...
            $response = [
                ...
                'form_type' => $form_type,
                'land_api' => $land_api,
                'emission' => $emission * $people * $nights,
    ...
    public function find_airport()
    {
        $db = \Config\Database::connect();
        $from = $this->request->getVar("departure");
        if (!$from) $from = $this->request->getVar("location_from");
        $to = $this->request->getVar("arrival");
        if (!$to) $to = $this->request->getVar("location_to");
        // echo " To: ".$to;
        // die();
        $apiKey = 'AIzaSyBB5u7MvYBi9uXF9ncpvJZbrW55a58QQMU';
        $formattedAddrFrom = str_replace(' ', '+', $from);
    ...
    public function find_location_and_airport_distance()
    {
        ...
        // print_r($airport_departure_transfer);
        // die;
        $query = $db->query("select * from company_vehicle_detail where vehicle=" . $airport_departure_transfer . " and status =1");
        $emission_id = $query->getResultArray()[0]['ghg_factor_id'];

        // print_r($emission_id);
        // die;
        $airport_departure_emission_factor = 0;
        $query = $db->query("select * from ghg_factor where id=" . $emission_id);
        ...
        // print_r($from);
        // print_r($departrue_airport_name);
        // die;
        // print_r($pureoutput);
        // die;
        ...
        // echo "<pre>";
        // print_r($pureoutput->rows[0]->elements[0]->distance->value);
        // die;
        $arr['distance'] = $distance . ' km';
        $emission_id = number_format($airport_departure_emission_factor * $distance, 3);
        $arr['emission'] = $emission_id . ' KgsCO2e';
        // print_r($arr['emission']);
        // die;
        $arr['emission_id'] = $emission_id;
    ...
    public function find_air_distance()
    {
        ...
        if ($err) {
            ...
            $arr['emission'] = number_format($rs->greatCircleDistance->km * $emission_factor, 2) . ' KgsCO2e';
            // print_r($arr['emission']);
            // die;
    ...
    public function find_distance()
    {
        ...
        // $api_key = 'AIzaSyBB5u7MvYBi9uXF9ncpvJZbrW55a58QQMU&libraries';
        // $latitude = '19.0760';
        // $longitude = '72.8777';
        // $radius = 500; // Radius in meters (adjust as needed)

        // // Create the URL to fetch nearby places using Google Places API
        // $url = "https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=$latitude,$longitude&radius=$radius&type=train_station&key=$api_key";
        ...
        //         echo "Station: $name<br>Address: $address<br><br>";
        //     }
        // } else {
        //     echo "Error: Unable to fetch station data.";
        // }
        // die;
        $db = \Config\Database::connect();
        ...
        $emission_factor_arr = $db->query("select emission_factor from ghg_factor where name ='" . $emission_id . "'")->getResultArray();
        // print_r($emission_id);
        // die;
        $emission_factor = 0;
        ...
        $arr = [];
        $emission = number_format(($distance * $emission_factor), 3);
        // $arr['emission'] = round($distance * $emission_factor);
        $arr['emission'] = $emission . ' KgsCO2e';
        $arr['emission_id'] = $emission;
        $arr['distance'] = $distance . ' km';
        // print_r($arr['emission']);
        // die;
        echo json_encode($arr);
    }
    public function footprint_report($id)
    {
        $rs = check_session();
        if (!$rs) {
            return redirect()->to('home/user_login');
        ...
        $ghg_id = $db->query("select * from control_footprints where id='" . $id . "' and status=1 ")->getResultArray()[0]['ghg'];
        $data['site_name'] = $db->query("select * from control_footprints as ab join supplier_assessment as bc where ab.sub_boundary=bc.id and ab.id='" . $id . "' and ab.status=1 ")->getResultArray()[0]['cp_name'];
        // print_r($site_name);
        // die;
        $ghg = new GhgModel();
        $data['ghg_name'] = $ghg->join('ghg_category', 'ghg_category.id=ghg.ghg_category_id')->where('ghg.id', $ghg_id)->where('ghg.status', 1)->first();
        $data['ghg_factor'] = $db->query("select * from ghg_factor as gf join factor as f where f.id=gf.name and f.ghg_id=$ghg_id and f.status=1 ")->getResultArray();
        // print_r($data['ghg_factor']);
        // die;
        $data['factor'] = $db->query("select * from factor where ghg_id='" . $ghg_id . "' and status=1 ")->getResultArray();
        $footprint = new Quantitative_Footprint_Model();
        $query = $footprint->where('footprint_id', $id)->where('status', 2)->first();
        // print_r($query);
        // die;
        $footprint_answer = new QuantitativeFootprintAnswerModel();
        $vehical = $footprint_answer->where('footprint_id', $id)->first();
        $vehical_id = $vehical['form_id'];
        // print_r($vehical_id);
        // die;
        if ($query['form_id'] == 9) {
            $data['foot'] = json_decode($query['quantity']);
            $data['quantitative_foot'] = json_decode($query['material']);
        } else {
            $data['foot'] = json_decode($query['value']);
            $data['quantitative_foot'] = json_decode($query['name']);
        }

        if ($vehical_id == '4') {
            $vehical_idd = $footprint_answer->where('form_id', $vehical_id)->first()['choose_vehicle'];
            // $data['ghg_factor'] = $db->query("select * from ghg_factor as gf join factor as f where f.id=gf.name and f.ghg_id=$ghg_id and f.status=1 ")->getResultArray();
            $data['ghg_factor'] = $db->query("select * from company_vehicle_detail as gf join ghg_factor as f join factor as fac where f.id=gf.ghg_factor_id and fac.id=f.name and gf.vehicle=$vehical_idd and f.status=1 and gf.status=1")->getResultArray();

            // print_r($data['ghg_factor']);
            // die;
            // $factor_id = $db->query("select * from company_vehicle_detail where vehicle='" . $id . "' and status=1 ")->getResultArray();
            $factor = $db->query("select * from company_vehicle_detail as gf join ghg_factor as f where f.id=gf.ghg_factor_id and gf.vehicle=$vehical_idd and f.status=1 and gf.status=1")->getResultArray()[0];
            // print_r($factor);
            // die;

            $data['quantitative_foot'] = [$factor['name']];
            $data['foot'] = [$vehical['mileage']];
        }
    ...
    public function get_unit($id)
    {
        ...
        return $model->where('id', $id)->first()['factor_unit'];
    }
 ...
 '" . $boundary . "','" . $uniq_spl_chr . "','" . $o_id . "','" . date("Y-m-d") . "')");
        ...
        if ($control_footprints) {
            ...
            $ghgCatShow = '';
            $ghgModel = new GhgModel();
            $ghgName = $ghgModel->whereIn('id', json_decode($ghgEncode))->findAll();
            foreach ($ghgName as $key => $value) {
                if ($key == 0) {
                    $ghgCatShow .= $value['ghg_name'];
                } else {
                    $ghgCatShow .= ', ' . $value['ghg_name'];
                }
            }

            $notimsg = 'New Task ' . $ghgCatShow . ' assigned to you by admin ' . $adminname['supplier_name'];
            // echo '<pre>'; print_r($notimsg); die;
            $noti = new UserNotification();
            $data = [
                'msg' => $notimsg,
                'created_by' => $supplier_info['supplier_id'],
                'owner_id' => $o_id,
                'for_y' => $for,
                'link' => 'Quantitative/control_footprints_submit'
            ];
            $noti->insert($data);
 ...
 For ' . ' Assessment on<b style="font-size:15px;"> POSITIIVPLUS</b> <b>' . ($detail['brand_name']) . '</b></p><a href="https://positiivplus.com/home/user_login" style="padding:8px 30px; color:black; border: 1px solid #caeb5d; border-radius: 5px; background: #defe73;text-decoration: none;display: inline-block;margin-top: 25px;font-size: 15px;">RECORD</a></div></div><div style="border-bottom-left-radius:10px;border-bottom-right-radius:10px;background:black; color:white; padding:20px;"><span style="color:white!important;">Copyright © ' . $time_date2 . ', All Right Reserved UTOPIIC</span><div style="float: right;margin-top: 5px;font-size: 10px;">' . $time_date1 . '</div><hr style="background: #4e4848;border: none;height: 1px;margin-top: 17px;"><div style="text-align: center;margin-top: 10px;"><a href="https://www.facebook.com/Utopiic/" style="margin-right: 15px;"><img src="https://positiivplus.com/public/socialicon/facebook.png" style="width: 17px;"></a><a href="https://www.instagram.com/utopiic.official/?hl=en" style="margin-right: 15px;"><img src="https://positiivplus.com/public/socialicon/instagram.png"></a><a href="https://twitter.com/utopiicofficial"><img src="https://positiivplus.com/public/socialicon/tw.png"></a></div></div></div>';
    ...
    public function updateAirTravel()
    {
        ...
        $footprint_answer = $footprint->where('footprint_id', $footprint_id)->where("(transport_type=$transport_type or transport_type=0)")->first();
        if ($footprint_answer) $save = $footprint->update($footprint_answer['id'], $data);
        else {
            $id = $footprint->insert($footprint->where("footprint_id=$footprint_id and owner_id=$o_id")->first());
            $save = $footprint->update($id, $data);
        }
        if ($save) {
            $response = [
                'success' => true,
    ...
    public function completeFootprint()
    {
        ...
        if ($this->request->getVar('form_type_9') == 9) {
            $footprint_answer = new Quantitative_Footprint_Model();
            // echo '1';
        } else {
            $footprint_answer = new QuantitativeFootprintAnswerModel();
            // echo '0';
        ...
        // $footprint_answer = new Quantitative_Footprint_Model();
        $footprint_answers = $footprint_answer->where("status=1 and owner_id=$o_id and footprint_id=$main_id")->first();
        // print_r($footprint_answers);
        // die;
        if ($footprint_answers) {
            $save = $footprint_answer->update($footprint_answers['id'], $data);
        }
        // print_r($save);
        // die;
        if ($save) {
 ...
 owner_id = '" . $o_id . "' WHERE id = $id ");
    ...
    public function answerFootprint_new($main_id)
    {
        $Qua_foo_model = new Quantitative_Footprint_Model();
        $supplier_model = new SupplierModel();
        $factor_id_arr = json_encode($this->request->getPost('factor_id_arr'));
        $arr_val = json_encode($this->request->getPost('arr_val'));
        $unit_id_arr = json_encode($this->request->getPost('unit_id_arr'));
        if (session()->supplier_info['role'] == 0) {
            $o_id = $u_id = session()->supplier_info['supplier_id'];
        } else {
            $u_id = session()->supplier_info['supplier_id'];
        ...
        $footprint_answer = $Qua_foo_model->where('status', 1)->where('owner_id', $o_id)->where('footprint_id', $main_id)->first();
        if ($footprint_answer) {
            $save = $Qua_foo_model->update($footprint_answer['id'], $data);
        } else {
            $save = $Qua_foo_model->save($data);
        }
        echo $save;
    }
    public function get_footprints($main_id)
    {
        $db = \Config\Database::connect();
        $arr = $db->query("SELECT * FROM quantitative_footprint_answer WHERE footprint_id=$main_id")->getResultArray();
        $data = '<table class="datatables" width="100%">
<thead><tr><th>#</th>
<th>Departure</th>
<th>Arrival</th>
<th>People</th>
...
<td>' . $v['people'] . '</td>
<td>' . $v['nights'] . '</td>
<td>' . $v['footprint_value'] . ' KgsCO2e</td>
