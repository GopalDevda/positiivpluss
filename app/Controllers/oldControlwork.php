...
$.ajax({
    ...
    public function assessment($num = null)
    {
        ...
        if ($num) {
            if ($num == 1) session()->set('qua', "Assessment");
            elseif ($num == 2) session()->set('qua', "Qualitative ");
        }
...
JOIN indicator_category AS indi ON indi.id= alq.indicator_id where alq.status = 1 AND ass.status=1 AND amaa.status = 1 AND ind.status = 1 AND alq.control_assessment_id = $main_id AND alq.indicator_id=$i_id"
    ...
    public function remark_guide($id)
    {

        $db = \Config\Database::connect();
        $assessment  = new AllAssessmentModel();
        $remark = $assessment->where('id', $id)->first();
        print_r($remark);
        die;
    }
    ...
    public function reset_sess()
    {
        return session()->remove('qua');
    }
}
