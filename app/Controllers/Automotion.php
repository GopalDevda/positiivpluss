...

{
    ...
    public function index()
    {
        ...
        $sensordata =   $sensor
            ->join('supplier_assessment', 'sensors.subboundary_id=supplier_assessment.id')
            ->where("sensors.status", 1)
            ->where("sensors.supplier_id = $u_id")->findAll();
        //     print_r($sensordata);
        //    die;
        ...
        // echo  max($dates);
        // die();
        //         $filename = "sensor.xlsk";
        //         $spread = new Spreadsheet();
        //         $sheet = $spread->getActiveSheet();
        //         $sheet->setCellValue('A1','cp_name');
        //         $sheet->setCellValue('B1','status');
        //         $rows = 1;
        //         foreach($sensordata as $eleid)
        //         {
        //             if($eleid['current_status'] == 1){
        //                 $statusname = 'Not Connect';
        //             }
        //             elseif($eleid['current_status'] == 2){
        ...
        //             $sheet->setCellValue('A'. $rows, $eleid['cp_name']);
        //             $sheet->setCellValue('B'. $rows, $statusname);
        //             $rows++;
        //         }
        //         $writer = new Xlsx($spread);
        //      $klll =  $writer->save($filename);
    ...
    public function sensor_data_view()
    {
        ...
        if ($filter) {
            ...
            $item =
                $sensor
                ->join("electricity_board", "sensors.provider_type=electricity_board.id")
                ->join('supplier_assessment', 'sensors.subboundary_id=supplier_assessment.id')
                ->like("supplier_assessment.cp_name", $filter)->where("sensors.status", 1)
                ->orLike("electricity_board.abbrevation", $filter)
                ->where("sensors.supplier_id = $u_id")
                ->like("electricity_board.abbrevation", $filter)->findAll();
        } else {
            // echo "Not in filter";
        ...
        foreach ($item as $key => $values) {
            ...
            if ($electricity_amount_id) {
                ...
                foreach ($electricity_amount_id as $key => $valuesfff) {
                    ...
                    $dkjg =  date('Y-m-d', $middle_id);
                    $month_name_show[] = $dkjg;
                    $month_name_show[$arr++];
                }

                array_multisort(array_map('strtotime', $month_name_show), $month_name_show);

                $latest_date =  $month_name_show[array_key_last($month_name_show)];
                $jdf =  max($month_name_show);
                $electricity_amount = $Data_electricity->where('status', 1)->where('electricity_id', $values['test_id'])->where('bill_date', $latest_date)->findAll();
            }
    ...
    public function new($id)
    {
        ...
        $electricity_data_fetch = $Dataelectricity->where('electricity_id', $id)->where('YEAR(bill_date)', $first)->where('MONTH(bill_date) >', 3)->where('status', 1)->Orwhere('YEAR(bill_date)', $second)->where('electricity_id', $id)->where('MONTH(bill_date) <=', 3)->where('status', 1)->findAll();
    ...
    public function financial($id)
    {
        ...
        $data['join_model'] = $Dataelectricity->where('electricity_id', $id)->where('status', 1)->orderBy('YEAR(bill_date)', 'DESC')->orderBy('MONTH(bill_date)', 'DESC')->findAll();
    ...
    public function bill_data_edit_fetch($id)
    {
        $model = new Data_electricityModel();
        $edit_data_view = $model->where('status', 1)->where('id', $id)->first();
        echo json_encode($edit_data_view);
    }

    public function editAutomotion()
    {
        ...
        $edit_pdfFile =  $this->request->getFile('edit_pdfFile');

        if (empty($_FILES['edit_pdfFile']['name'])) {
            $bill_name = $this->request->getVar('edit_bill_name');
            $newNameD = $this->request->getVar('edit_pdf_name');
        } else {
            $input = $this->validate([
                'edit_pdfFile' => 'uploaded[edit_pdfFile]|ext_in[edit_pdfFile,pdf],'
            ]);

            $l = $edit_pdfFile->getName();
            $bill_name  = rtrim($l, ".pdf");

            if ($input) {
                if ($edit_pdfFile->isValid() && !$edit_pdfFile->hasMoved()) {

                    $newNameD = $edit_pdfFile->getRandomName();
                    $edit_pdfFile->move('public/uploads/pdfElectricity', $newNameD);
                }
            }
        }


        $monthly =   json_encode($this->request->getVar('edit_monthly_name'));
        $t = time();
        $currentdatetime =   date("Y-m-d H:i:s", $t);

        $ed_id = $this->request->getVar('edit_id');
        $data =
            [
                // 'electricity_id' => $this->request->getVar('edit_id'),
                'bill_no' => $bill_name,
                'bill_date' => $this->request->getVar('edit_bill_date'),
                'consume_unit' => $this->request->getVar('edit_consume_unit'),
                'demand_load' => $this->request->getVar('edit_demand_load'),
                'senstion' => $this->request->getVar('edit_senstion'),
                'power_load' => $this->request->getVar('edit_power_load'),
                'year' => $this->request->getVar('edit_year'),
                'amount' => $this->request->getVar('edit_amount'),
                'frequency' => $this->request->getVar('edit_frequency'),
    ...
    public function checkSiteExists()
    {
        $db = \Config\Database::connect();
        $session = session();
        $sensor_model = new SensorModel();
        $sub_boundary = $this->request->getVar('subboundary_id');
        $sub_boun = explode("/", $sub_boundary)[0];
        $sub_site = $this->request->getVar('sub_site');
        print_r($sub_site);
        print_r($sub_boun);
        die();
        $u_id = session()->supplier_info['supplier_id'];
        $same_site = $sensor_model->where('supplier_id', $u_id)->where('subboundary_id', $sub_boun)->where('sub_site', $sub_site)->where('status', 1)->first();
        print_r($same_site);
        die;
        if ($same_site) {
            $exits = ['true' => '1'];
    ...
    public function automation_cron()
    {
        $db = \Config\Database::connect();
        $session = session();
        $supplier_info = $session->get('supplier_info');
        $id = $supplier_info['supplier_id'];
        $automation_model = new  SensorModel();
        $fetch_query = $automation_model->where('supplier_id', $id)->where('username', '1006405439')->where('password', 'Aubank@123')->where('current_status', 2)->where('status', 1)->first();
        print_r($fetch_query);
        die;
    }
}
