...
class Supplier extends BaseController
{
	...
	public function index()
	{
		...
		if (session()->supplier_info['role'] == 0) {
			$sites_count = $db->query("SELECT * FROM supplier_assessment where is_submit=1 AND owner_id = $o_id AND assessment_id =12")->getResultArray();
			$product_count = $db->query("SELECT * FROM supplier_assessment where is_submit=1 AND owner_id = $o_id AND assessment_id =11")->getResultArray();
			$supplier_count = $db->query("SELECT * FROM supplier_type where status=1 AND owner_id = $o_id ")->getResultArray();
		} else {
			$sites_count = $db->query("SELECT * FROM supplier_assessment where is_submit=1 AND supplier_id = $u_id AND assessment_id =12")->getResultArray();
			$product_count = $db->query("SELECT * FROM supplier_assessment where is_submit=1 AND supplier_id = $u_id AND assessment_id =11")->getResultArray();
			$supplier_count = $db->query("SELECT * FROM supplier_type where status=1 AND supplier_id = $u_id ")->getResultArray();
		}

		$data['countSite'] = count($sites_count);

		$data['countProduct'] = count($product_count);

		$data['countSupplier'] = count($supplier_count);

		$environemt = new Energy_managment();
		$quickStart = new QuickStartModel();

		$employe_query = $quickStart->where('owner_id', $o_id)->first();

		$emp_div = json_decode($employe_query['employee_per']);
		$emp_value = json_decode($employe_query['employee_total']);
		$turnover = $employe_query['turnover'];
		$empSum = 0;
		if ($emp_div) {
			foreach ($emp_div as $key => $empId) {

				foreach ($emp_value as $key1 => $empvalue) {
					if ($empId == 1) {
						if ($key == $key1) {
							$empSum += $empvalue;
						}
		...
		$emissionsum = 0;
		$emission_query = $environemt->where('supplier_id', $o_id)->where('status', 1)->where('indicator_id', 20)->findAll();
		//  print_r($emission_query);
		//  die;
		if ($emission_query) {
			foreach ($emission_query as $emi_value) {
				$emissionsum +=  $emi_value['value'];
			}
		}
		$data['emissiontotal'] = $emissionsum;


		if ($empSum) {
			$data['emission_per_emp'] = $emissionsum / $empSum;
		} else {
			$data['emission_per_emp'] = 0;
		}


		if (empty($turnover)) {
			$data['emission_per_turnover'] = 0;
		} else {
			$data['emission_per_turnover'] = $emissionsum / $turnover;
		}
		// Start  Renewable Graph Query Dynamic
		$ren = 0;
		$energy_renewable = $environemt->select('id,sub_e_type,value,monthly_name')->where('supplier_id', $o_id)->where('status', 1)->where('indicator_id', 17)->where('data_id', 3)->findAll();

		foreach ($energy_renewable as $energy_renewable_val) {
			$ren_month = json_decode($energy_renewable_val['monthly_name']);
			foreach ($ren_month as $month) {
				$ren_month_name[] = $month;
				$ren_month_name[$ren++];
			}
		}
		...
		$ren_Nov = 0;
		$ren_Dec = 0;
		// print_r($ren_month_name);
		// die;
		if ($ren_month_name) {
			$month_name_show_ren = array_unique($ren_month_name);
			foreach ($month_name_show_ren as $month_ren) {
				$ren_ID = '"' . "$month_ren" . '"';
				$renewable_query = $environemt->select('id,sub_e_type,value,monthly_name')->where('supplier_id', $o_id)
					->where('status', 1)->where('indicator_id', 17)->where('data_id', 3)->like('monthly_name', $ren_ID)->findAll();

				foreach ($renewable_query as $key => $renewable_query_id) {
					if ($month_ren == '1') {
						$ren_jan += $renewable_query_id['value'];
					}
					if ($month_ren == '2') {
						$ren_Feb += $renewable_query_id['value'];
					}
					if ($month_ren == '3') {
						$ren_Mar += $renewable_query_id['value'];
					}
					if ($month_ren == '4') {
						$ren_Apr += $renewable_query_id['value'];
					}
					if ($month_ren == '5') {
						$ren_May += $renewable_query_id['value'];
					}
					if ($month_ren == '6') {
						$ren_jun += $renewable_query_id['value'];
					}
					if ($month_ren == '7') {
						$ren_july += $renewable_query_id['value'];
					}
					if ($month_ren == '8') {
						$ren_Aug += $renewable_query_id['value'];
					}
					if ($month_ren == '9') {
						$ren_Sep += $renewable_query_id['value'];
					}
					if ($month_ren == '10') {
						$ren_Oct += $renewable_query_id['value'];
					}
					if ($month_ren == '11') {
						$ren_Nov += $renewable_query_id['value'];
					}
					if ($month_ren == '12') {
						$ren_Dec += $renewable_query_id['value'];
					}
				}
			}
		...
		// print_r($data1);
		// die;
		// End  Renewable Graph Query Dynamic
		// Start non Renewable Graph Query Dynamic

		$nonren = 0;
		$energy_non_renewable = $environemt->select('id,sub_e_type,value,monthly_name')->where('supplier_id', $o_id)->where('status', 1)->where('indicator_id', 17)->where('data_id', 2)->findAll();
		foreach ($energy_non_renewable as $energy_non_renewable_val) {
			$nonren_month = json_decode($energy_non_renewable_val['monthly_name']);
			foreach ($nonren_month as $monthnon) {
				$nonren_month_name[] = $monthnon;
				$nonren_month_name[$nonren++];
			}
		}
		...
		$nonren_Nov = 0;
		$nonren_Dec = 0;
		if ($nonren_month_name) {
			$month_name_show_nonren = array_unique($nonren_month_name);
			foreach ($month_name_show_nonren as $month_nonren) {
				$nonren_ID = '"' . "$month_nonren" . '"';
				$nonrenewable_query = $environemt->select('id,sub_e_type,value,monthly_name')->where('supplier_id', $o_id)
					->where('status', 1)->where('indicator_id', 17)->where('data_id', 2)->like('monthly_name', $nonren_ID)->findAll();

				foreach ($nonrenewable_query as $key => $nonrenewable_query_id) {

					if ($month_nonren == '1') {
						$nonren_jan += $nonrenewable_query_id['value'];
					}
					if ($month_nonren == '2') {
						$nonren_Feb += $nonrenewable_query_id['value'];
					}
					if ($month_nonren == '3') {
						$nonren_Mar += $nonrenewable_query_id['value'];
					}
					if ($month_nonren == '4') {
						$nonren_Apr += $nonrenewable_query_id['value'];
					}
					if ($month_nonren == '5') {
						$nonren_May += $nonrenewable_query_id['value'];
					}
					if ($month_nonren == '6') {
						$nonren_jun += $nonrenewable_query_id['value'];
					}
					if ($month_nonren == '7') {
						$nonren_july += $nonrenewable_query_id['value'];
					}
					if ($month_nonren == '8') {
						$nonren_Aug += $nonrenewable_query_id['value'];
					}
					if ($month_nonren == '9') {
						$nonren_Sep += $nonrenewable_query_id['value'];
					}
					if ($month_nonren == '10') {
						$nonren_Oct += $nonrenewable_query_id['value'];
					}
					if ($month_nonren == '11') {
						$nonren_Nov += $nonrenewable_query_id['value'];
					}
					if ($month_nonren == '12') {
						$nonren_Dec += $nonrenewable_query_id['value'];
					}
				}
			}
		...
		//    die;
		// End Non Renewable Graph Query Dynamic
		...
		$arr = 0;
		if ($scope1) {
			$Scope1_query = $environemt->select('id,sub_e_type,value,monthly_name')->where('supplier_id', $o_id)->where('status', 1)->where('indicator_id', 20)->where('disclosure_id', $scope1)->findAll();
			foreach ($Scope1_query as $SId) {
				$month_name = json_decode($SId['monthly_name']);
				foreach ($month_name as $month) {
					$month_name_show[] = $month;
					$month_name_show[$arr++];
				}
			}
			$jan = 0;
			...
			$Dec = 0;
			if ($month_name_show) {
				$month_name_show_id = array_unique($month_name_show);
				foreach ($month_name_show_id as $month_n) {
					$ID = '"' . "$month_n" . '"';
					$Scope1_queryo = $environemt->select('id,sub_e_type,value,monthly_name')->where('supplier_id', $o_id)
						->where('status', 1)->where('indicator_id', 20)->where('disclosure_id', $scope1)->like('monthly_name', $ID)->findAll();
					foreach ($Scope1_queryo as $key => $Scope1_queryo_id) {
						if ($month_n == '1') {
							$jan += $Scope1_queryo_id['value'];
						}
						if ($month_n == '2') {
							$Feb += $Scope1_queryo_id['value'];
						}
						if ($month_n == '3') {
							$Mar += $Scope1_queryo_id['value'];
						}
						if ($month_n == '4') {
							$Apr += $Scope1_queryo_id['value'];
						}
						if ($month_n == '5') {
							$May += $Scope1_queryo_id['value'];
						}
						if ($month_n == '6') {
							$jun += $Scope1_queryo_id['value'];
						}
						if ($month_n == '7') {
							$july += $Scope1_queryo_id['value'];
						}
						if ($month_n == '8') {
							$Aug += $Scope1_queryo_id['value'];
						}
						if ($month_n == '9') {
							$Sep += $Scope1_queryo_id['value'];
						}
						if ($month_n == '10') {
							$Oct += $Scope1_queryo_id['value'];
						}
						if ($month_n == '11') {
							$Nov += $Scope1_queryo_id['value'];
						}
						if ($month_n == '12') {
							$Dec += $Scope1_queryo_id['value'];
						}
					}
				}
			}
		...
		$arr2 = 0;
		if ($scope2) {
			$Scope2_query = $environemt->select('id,sub_e_type,value,monthly_name')->where('supplier_id', $o_id)->where('status', 1)->where('indicator_id', 20)->where('disclosure_id', $scope2)->findAll();
			foreach ($Scope2_query as $SId1) {
				$month_name1 = json_decode($SId1['monthly_name']);
				foreach ($month_name1 as $month1) {
					$month_name_show1[] = $month1;
					$month_name_show1[$arr2++];
				}
			}
			$jan1 = 0;
			$Feb1 = 0;
			...
			$Dec1 = 0;
			if ($month_name_show1) {
				$month_name_show_id = array_unique($month_name_show1);
				foreach ($month_name_show_id as $month_n1) {
					$ID1 = '"' . "$month_n1" . '"';
					$Scope2_queryo = $environemt->select('id,sub_e_type,value,monthly_name')->where('supplier_id', $o_id)
						->where('status', 1)->where('indicator_id', 20)->where('disclosure_id', $scope2)->like('monthly_name', $ID1)->findAll();
					// print_r($Scope2_queryo);
					foreach ($Scope2_queryo as $key => $Scope2_queryo_id) {

						if ($month_n1 == '1') {
							$jan1 += $Scope2_queryo_id['value'];
						}
						if ($month_n1 == '2') {
							$Feb1 += $Scope2_queryo_id['value'];
						}
						if ($month_n1 == '3') {
							$Mar1 += $Scope2_queryo_id['value'];
						}
						if ($month_n1 == '4') {
							$Apr1 += $Scope2_queryo_id['value'];
						}
						if ($month_n1 == '5') {
							$May1 += $Scope2_queryo_id['value'];
						}
						if ($month_n1 == '6') {
							$jun1 += $Scope2_queryo_id['value'];
						}
						if ($month_n1 == '7') {
							$july1 += $Scope2_queryo_id['value'];
						}
						if ($month_n1 == '8') {
							$Aug1 += $Scope2_queryo_id['value'];
						}
						if ($month_n1 == '9') {
							$Sep1 += $Scope2_queryo_id['value'];
						}
						if ($month_n1 == '10') {
							$Oct1 += $Scope2_queryo_id['value'];
						}
						if ($month_n1 == '11') {
							$Nov1 += $Scope2_queryo_id['value'];
						}
						if ($month_n1 == '12') {
							$Dec1 += $Scope2_queryo_id['value'];
						}
					}
				}
			}
		...
		$arr3 = 0;
		if ($scope3) {
			$Scope3_query = $environemt->select('id,sub_e_type,value,monthly_name')->where('supplier_id', $o_id)->where('status', 1)->where('indicator_id', 20)->where('disclosure_id', $scope3)->findAll();
			foreach ($Scope3_query as $SId2) {
				$month_name2 = json_decode($SId2['monthly_name']);
				foreach ($month_name2 as $month2) {
					$month_name_show2[] = $month2;
					$month_name_show2[$arr3++];
				}
			}
			$jan2 = 0;
			$Feb2 = 0;
			...
			$Dec2 = 0;
			if ($month_name_show2) {
				$month_name_show_id = array_unique($month_name_show2);
				foreach ($month_name_show_id as $month_n2) {
					$ID2 = '"' . "$month_n2" . '"';
					$Scope3_queryo = $environemt->select('id,sub_e_type,value,monthly_name')->where('supplier_id', $o_id)
						->where('status', 1)->where('indicator_id', 20)->where('disclosure_id', $scope3)->like('monthly_name', $ID2)->findAll();
					// print_r($Scope2_queryo);
					foreach ($Scope3_queryo as $key => $Scope3_queryo_id) {

						if ($month_n2 == '1') {
							$jan2 += $Scope3_queryo_id['value'];
						}
						if ($month_n2 == '2') {
							$Feb2 += $Scope3_queryo_id['value'];
						}
						if ($month_n2 == '3') {
							$Mar2 += $Scope3_queryo_id['value'];
						}
						if ($month_n2 == '4') {
							$Apr2 += $Scope3_queryo_id['value'];
						}
						if ($month_n2 == '5') {
							$May2 += $Scope3_queryo_id['value'];
						}
						if ($month_n2 == '6') {
							$jun2 += $Scope3_queryo_id['value'];
						}
						if ($month_n2 == '7') {
							$july2 += $Scope3_queryo_id['value'];
						}
						if ($month_n2 == '8') {
							$Aug2 += $Scope3_queryo_id['value'];
						}
						if ($month_n2 == '9') {
							$Sep2 += $Scope3_queryo_id['value'];
						}
						if ($month_n2 == '10') {
							$Oct2 += $Scope3_queryo_id['value'];
						}
						if ($month_n2 == '11') {
							$Nov2 += $Scope3_queryo_id['value'];
						}
						if ($month_n2 == '12') {
							$Dec2 += $Scope3_queryo_id['value'];
						}
					}
				}
			}
		...
		$fuel3 = 0;
		if ($totalFuel) {
			$fuel_query = $environemt->select('id,sub_e_type,value,monthly_name')->where('supplier_id', $o_id)->where('status', 1)->where('indicator_id', 17)->where('sub_disclosure_id', $totalFuel)->findAll();
			foreach ($fuel_query as $fuel) {
				$month_fuel = json_decode($fuel['monthly_name']);
				foreach ($month_fuel as $monthfuel) {
					$month_name_data[] = $monthfuel;
					$month_name_data[$fuel3++];
				}
			}
			$janfuel = 0;
			$Febfuel = 0;
			...
			$Decfuel = 0;
			if ($month_name_data) {
				$month_name_fuel = array_unique($month_name_data);
				foreach ($month_name_fuel as $month_fuel) {
					$fuel2 = '"' . "$month_fuel" . '"';
					$fuel_queryo = $environemt->select('id,sub_e_type,value,monthly_name')->where('supplier_id', $o_id)
						->where('status', 1)->where('indicator_id', 17)->where('sub_disclosure_id', $totalFuel)->like('monthly_name', $fuel2)->findAll();
					// print_r($Scope2_queryo);
					foreach ($fuel_queryo as $key => $fuel_queryo_id) {

						if ($month_fuel == '1') {
							$janfuel += $fuel_queryo_id['value'];
						}
						if ($month_fuel == '2') {
							$Febfuel += $fuel_queryo_id['value'];
						}
						if ($month_fuel == '3') {
							$Marfuel += $fuel_queryo_id['value'];
						}
						if ($month_fuel == '4') {
							$Aprfuel += $fuel_queryo_id['value'];
						}
						if ($month_fuel == '5') {
							$Mayfuel += $fuel_queryo_id['value'];
						}
						if ($month_fuel == '6') {
							$junfuel += $fuel_queryo_id['value'];
						}
						if ($month_fuel == '7') {
							$julyfuel += $fuel_queryo_id['value'];
						}
						if ($month_fuel == '8') {
							$Augfuel += $fuel_queryo_id['value'];
						}
						if ($month_fuel == '9') {
							$Sepfuel += $fuel_queryo_id['value'];
						}
						if ($month_fuel == '10') {
							$Octfuel += $fuel_queryo_id['value'];
						}
						if ($month_fuel == '11') {
							$Novfuel += $fuel_queryo_id['value'];
						}
						if ($month_fuel == '12') {
							$Decfuel += $fuel_queryo_id['value'];
						}
					}
				}
			}
		...
		$data['monthlyfuel'] = $fuelquery;


		// Fuel Consumption End 

		$totalElect = '29';
		$Electi = 0;
		if ($totalElect) {
			$Elect_query = $environemt->select('id,sub_e_type,value,monthly_name')->where('supplier_id', $o_id)->where('status', 1)->where('indicator_id', 17)->where('sub_disclosure_id', $totalElect)->findAll();
			foreach ($Elect_query as $Elect) {
				$month_Elect = json_decode($Elect['monthly_name']);
				foreach ($month_Elect as $monthElect) {
					$month_name_monthElect[] = $monthElect;
					$month_name_monthElect[$Electi++];
				}
			}
			$janElect = 0;
			$FebElect = 0;
			...
			if ($month_name_monthElect) {
				$month_name_elect = array_unique($month_name_monthElect);
				foreach ($month_name_elect as $month_elect) {
					$elct = '"' . "$month_elect" . '"';
					$elect_queryo = $environemt->select('id,sub_e_type,value,monthly_name')->where('supplier_id', $o_id)
						->where('status', 1)->where('indicator_id', 17)->where('sub_disclosure_id', $totalElect)->like('monthly_name', $elct)->findAll();
					// print_r($Scope2_queryo);
					foreach ($elect_queryo as $key => $elct_queryo_id) {

						if ($month_elect == '1') {
							$janElect += $elct_queryo_id['value'];
						}
						if ($month_elect == '2') {
							$FebElect += $elct_queryo_id['value'];
						}
						if ($month_elect == '3') {
							$MarElect += $elct_queryo_id['value'];
						}
						if ($month_elect == '4') {
							$AprElect += $elct_queryo_id['value'];
						}
						if ($month_elect == '5') {
							$MayElect += $elct_queryo_id['value'];
						}
						if ($month_elect == '6') {
							$junElect += $elct_queryo_id['value'];
						}
						if ($month_elect == '7') {
							$julyElect += $elct_queryo_id['value'];
						}
						if ($month_elect == '8') {
							$AugElect += $elct_queryo_id['value'];
						}
						if ($month_elect == '9') {
							$SepElect += $elct_queryo_id['value'];
						}
						if ($month_elect == '10') {
							$OctElect += $elct_queryo_id['value'];
						}
						if ($month_elect == '11') {
							$NovElect += $elct_queryo_id['value'];
						}
						if ($month_elect == '12') {
							$DecElect += $elct_queryo_id['value'];
						}
					}
				}
			}
		...
		$data['monthlyElect'] = $Electquery;


		// print_r($data['monthlyElect']);
		// die;

		// print_r($data['emission_per_turnover']);
		// die;

		// $data['emission_per_emp'] = number_format($emission_per_emp,2);
	...
	function market_place()
	{
		...
		$uniqueNames = array();
		$db = \Config\Database::connect();
		$datas = $db->query("SELECT * FROM `madhyapradesh` WHERE status=1")->getResultArray(500);
		$i = 0;
		foreach ($datas as $item) {
			$name = $item['principal_bussiness_activity_as_per_cin'];
	...
	public function BasicInfo()
	{
		...
		// print_r($socials);
		// die();
		$trunover = $this->request->getVar('trunover');
		...
		$s_date = ['success' => 'Basic Info successfully Added'];
		return $this->response->setJSON($s_date);
	}
	...
	public function signupstep($supplier_id, $plan_id)
	{
		...
		if (isset($supplier_id) && $supplier_id != '') {
			...
			$data['industry_category'] = $db->query('select * from industry_category where status=1 and id!=17')->getResultArray();
			// print_r(count($data['industry_category']));
			// die;
			// $data['country'] = $query->getResultArray();
			$data['plan_id'] = $plan_id;
	...
	public function industry_category($id)
	{
		...
		$db = \Config\Database::connect();
		$indi = new IndustryModel();
		$data = $indi->where('industry_category_id', $id)->where('status', 1)->findAll();
		return $this->response->setJSON($data);
	}
	...
	public function signupStepSubmit()
	{
		...
		$name = $model->where('id', $supplier_id)->first()['supplier_name'];
		...
		if ($insert) {
			$query = $db->query("update supplier_type set company_name='" . $brand_name . "',state='" . $state . "',country='" . $country . "',name='" . $name . "' where email='" . $model->select('email')->where('id', $supplier_id)->first()['email'] . "' ");

			$l = $db->query("update supplier_type set supplier_industry=$industry_id where email='" . $model->select('email')->where('id', $supplier_id)->first()['email'] . "'");
			$query = $db->query(
				"select * from supplier where  id='" . $supplier_id . "' order by id asc"
			);
			// print_r($l);
			// die;
	...
	public function document()
	{
		...
		$totalSize = 0;
		$documentData = $db->query("select * from supplier_document where supplier_id='" . $supplier_info['supplier_id'] . "'  and status=1")->getResultArray();
		foreach ($documentData as $key => $docData) {
			$totalSize += $docData['file_size'];
		}

		$data['limitsize'] = $totalSize;

		//  print_r($totalSize);
		//  die;
		$query = $db->query("select * from document_Storage where user_id='" . $supplier_info['supplier_id'] . "'");
		...
		if ($totalSize == 0) {
			...
			$docPer = number_format(($totalSize / $limi) * 100, '2');
		}
		$data['docper'] = $docPer;
	...
	public function addDocument()
	{
		...
		if ($totalSize > $limi) {
			$session->setFlashdata('error', 'Document Library is full');
		...
		return redirect()->to('company_document');
	}
	...
	public function quickStart()
	{
		...
		$data['turnover'] =  $find_total['turnover'];
		if ($find_total) {
	...
	public function readallnotification()
	{
		...
		$where = "owner_id=$o_id and (for_y=$u_id or for_y=1) and status=1";
		$data['notiData'] = $noti->where($where)->orderBy('id', 'DESC')->findAll();
		// echo '<pre>'; print_r($data); 
		echo view('brand/notification', $data);
	}
	public function company_design()
	{
		$rs = check_session();
		if (!$rs) {
			return redirect()->to('home/user_login');
		...
		echo view('brand/company_profiledesign', $data);
	}
}
