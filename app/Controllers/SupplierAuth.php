...
class SupplierAuth extends BaseController
{
    ...
    public function otpverify()
    {
        ...
        <img src="https://positiivplus.com/public/utopiic/assets/app-assets/images/logo/logo.png"></div>
            ...
            <div style="padding:40px; font-size:18px;border: 1px solid #ddd;">
                <div>
                    ...
                    <p style="margin-top:0px;margin-bottom:13px;">Welcome to <b>POSITIIVPLUS</b> - the one-stop shop for all your ESG (Environmental, Social, and Governance) needs. Our cloud-based digital platform was designed to make it easier than ever before to manage ESG within your organization, so that you can take the necessary steps towards sustainability while also achieving your desired outcomes.<br>
                        ...
                        <br>
                        Login Link: https://positiivplus.com/home/user_login<br>
        ...
        <div style="border-bottom-left-radius:10px;border-bottom-right-radius:10px;background:black; color:white; padding:20px;">
            ...
            <div style="text-align: center;margin-top: 10px;">
                <a href="https://www.facebook.com/Utopiic/" style="margin-right: 15px;"><img src="https://positiivplus.com/public/socialicon/facebook.png" style="width: 17px;"></a>
                <a href="https://www.instagram.com/utopiic.official/?hl=en" style="margin-right: 15px;"><img src="https://positiivplus.com/public/socialicon/instagram.png" style="width: 17px;"></a>
                <a href="https://twitter.com/utopiicofficial"><img src="https://positiivplus.com/public/socialicon/tw.png" style="width: 17px;"></a>
            </div>
        </div>
    </div>';
            $message .= '</html></body>';
            ...
            $email->setFrom('info@positiivplus.com', 'PositiivPlus Onboarding');
    ...
    public function signuppp()
    {
        ...
        if ($password == $cpassword) {
            ...
            if (!empty($ava)) {
                ...
                $sup_ty = new SupplierType(); //Add Suppliertype model path at the top
                $sup_data = $sup_ty->where('email', $receiptemail)->first();
                ...
                if (!empty($sup_data)) {
                    ...
                    $detail = $supplier->where('id', $sup_data['supplier_id'])->first();
                    $role = $detail['role'];
                    $name_admin = $detail['supplier_name'];
                    if ($role == '10' || $role == '0') {
                        $role_name = 'Admin';
                    }
                    if ($role == '11') {
                ...
                // $otp = 12345;
                $supplier_id = $model->insert(['supplier_name' => $name, 'mobile' => $mobile, 'otp' => $otp, 'email' => $receiptemail, 'password' => password_hash($password, PASSWORD_DEFAULT), 'supplier_plan_id' => $plan, 'pass' => $password, 'supplier_paid' => $paid]);
                if ($supplier_id) {
        ...
        <div style="border-bottom-left-radius:10px;border-bottom-right-radius:10px;background:black; color:white; padding:20px;">
            ...
            <div style="text-align: center;margin-top: 10px;">
                ...
                <a href="https://twitter.com/utopiicofficial"><img src="https://positiivplus.com/public/socialicon/tw.png" style="width: 17px;"></a>
                    ...
                    // print_r($message);die();
                    $email = \Config\Services::email();
                    $email->setFrom('info@positiivplus.com', 'PositiivPlus Verification');
                    $email->setTo($receiptemail);
                    // $email->mailType(html);
    ...
    public function login()
    {
        ...
        $login_active = $rs['login_active'];
        if ($login_active == 1) {
            $session->setFlashdata("error", "User Already Login");
    ...
    public function login_otpverify()
    {
        ...
        if ($otp == "6978") {
            ...
            // code for login
            $empData = $supplier_model->where('id', $o_id)->first();
            $empName = $empData['supplier_name'];
            $role = $empData['role'];
            ...
            $adminId = $empData['added_by'];
            if ($adminId) {
                $adminData = $supplier_model->where('id', $adminId)->first();
                $adminName = $adminData['supplier_name'];
                $adminEmail = $adminData['email'];
                $notimsg = $empName . ' Employee Onboarding Complete';
                $noti = new UserNotification();
                $msgCheck = $noti->where('created_by', $o_id)->where('msg', $notimsg)->first();

                if (!$msgCheck) {
                    //start onboarding admin notification
                    $data = [
                        'msg' => $notimsg,
                        'created_by' => $o_id,
                        'owner_id' => $adminId,
                        'for_y' => $adminId,
                        'link' => 'SupplierAuth/login'
                    ];
                    $noti->insert($data);
                    //end onboarding admin notification
                    //start onboarding admin email
                    $subject = 'SUB: Employee Onboarding Completed! ' . date("Y-m-d-h-i-s");
                    ...
                    $admin_message = '';
                    $admin_message .= '<html><body>';
                    $admin_message .= '<div style="margin: 0 auto;width: 600px;"><div style="background:black;padding:22px;border-top-right-radius:10px;border-top-left-radius:10px;"><img src="https://positiivplus.com/public/utopiic/assets/app-assets/images/logo/logo.png"></div><div style="background:white;"><div style="padding:40px; font-size:18px;border: 1px solid #ddd;"><h3 style="margin-top:0px;margin-bottom:13px;">Hello ' . $adminName . ',</h3><p><br><hr style="color: #ddd;background: #f1f1f1;height: 1px;border: none;"><h3 style="margin-bottom:0px; margin-top:13px;"> Employee Onboarding Completed </h3> <h4 style="margin-bottom:0px; margin-top:5px;">' . $empName . ' has successfully added</h4><p  style="margin-bottom:13px; margin-top:13px;">
                        ' . '</div></div><div style="border-bottom-left-radius:10px;border-bottom-right-radius:10px;background:black; color:white; padding:20px;"><span style="color:white!important;">Copyright © ' . $time_date2 . ', All Right Reserved UTOPIIC</span><div style="float: right;margin-top: 5px;font-size: 10px;">' . $time_date1 . '</div><hr style="background: #4e4848;border: none;height: 1px;margin-top: 17px;"><div style="text-align: center;margin-top: 10px;"><a href="https://www.facebook.com/Utopiic/" style="margin-right: 15px;"><img src="https://positiivplus.com/public/socialicon/facebook.png"  style="width: 17px;"></a><a href="https://www.instagram.com/utopiic.official/?hl=en" style="margin-right: 15px;"><img src="https://positiivplus.com/public/socialicon/instagram.png"></a><a href="https://twitter.com/utopiicofficial"><img src="https://positiivplus.com/public/socialicon/tw.png"></a></div></div></div>';
                    $admin_message .= '</body></html>';
                    // echo $admin_message;
                    // die;
                    $email = \Config\Services::email();
                    $email->setFrom('info@positiivplus.com', 'Employee Onboarding');
                    $email->setTo($adminEmail);
                    // $email->mailType(html);
                    $email->setSubject('Onboarding');
                    $email->setMessage($admin_message);
                    $a = $email->send();
                    //end onboarding admin email
                }
            }
            ...
            if ($profile) {
                ...
                if ($role == 11) {
                    if ($model['designation'] == '' || $model['employee_code'] == '' || $model['department'] == '') {
