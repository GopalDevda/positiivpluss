...
# Get the current month as a number (1 to 12)
...
for F_path in Path(folder_path).glob("*.pdf"):
    ...
    query = "SELECT id FROM sensors WHERE provider_type = '73' AND login_status = '2' AND kn_no ={} AND download_month !='{}' ;".format(Acc_no,current_month)
    ...
    # Extract the id value as a string from the row
        ...
        if not existing_data:
            ...
            db.commit()
            query1 = "SELECT username,password FROM sensors WHERE provider_type = '73' AND login_status = '2' AND kn_no ={} AND id ='{}' ;".format(Acc_no,id_value)
            cursor.execute(query1)
            rows = cursor.fetchall()
            for row in rows:
                username, password= row
                update_status_query = "UPDATE sensors SET login_status = '2' , current_status = '3' ,download_month = '{}'  WHERE Username = %s AND password = %s AND kn_no = %s;".format(current_month)
                cursor.execute(update_status_query, (username,password,Acc_no,))
