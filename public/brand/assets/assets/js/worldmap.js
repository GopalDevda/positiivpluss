...
// Enable animations, e.g. fade the show/hide of labels
am4core.useTheme(am4themes_animated); // This comes from themes/animated.js

// Create map instance
var chart = am4core.create("chartmap", am4maps.MapChart);

// Set map definition
chart.geodata = am4geodata_worldLow; // This comes from geodata/worldLow.js
...
// Make map load polygon data (e.g. country names) from GeoJSON
polygonSeries.useGeodata = true;

// Configure default look, feel, behavior, and properties of countries via template
...
// How much pixel space (the surface area of a rectangle in pixels) should a
// country occupy on the screen for its label to become visible?
var visibilityThreshold = 2500;

// Calculate pixel surface area of a polygon on the map
function calculateMapPolygonSquarePx(polygon) {
  
  // rudimentary surface area of map polygon by grabbing far most north + east and south + west coordinates,
  // the difference between west and east, south and north, gives us 2 sides of a rectangle,
  // multiplying those gives us its area.
  
  var topLeft = chart.geoPointToSVG({
    longitude: polygon.dataItem.west,
    latitude: polygon.dataItem.north
  });
  var bottomRight = chart.geoPointToSVG({
    longitude: polygon.dataItem.east,
    latitude: polygon.dataItem.south
  });
  
  // If the polygon is offmap, some of the values could be < 0,
  // so we use Math.abs to ensure a positive value for surface area.
  return Math.abs(
...
// Update an individual country's current surface area in px
// 
// fn is an optional callback to run against the country, namely updating its
// label's visibility.
function updateSurfaceAreaPxData(mapPolygon, fn) {
  if ( ! mapPolygon.dummyData) {
    mapPolygon.dummyData = {}; // Though we can do whatever we want w/ dummyData, by default it doesn't exist as a property yet
  }
  
  mapPolygon.dummyData.surfaceAreaPx = calculateMapPolygonSquarePx(mapPolygon);
...
// Update all countries' current surface area in px, one country at a time.
// 
// For details on the parameter fn, see the function above.
function updateAllSurfaceAreaPxData(fn) {  
  polygonSeries.mapPolygons.each(function(mapPolygon) { // should this be further optimized? it will needlessly run dozens of times on a zoom.
    updateSurfaceAreaPxData(mapPolygon, fn);
  });
}

// Update individual label or all labels' visibility via Sprite State
// 
// If a mapPolygon is provided, just update its related label, otherwise update
// all labels. Updating all labels should only happen once, though!
function updateLabelVisibility(countryMapPolygon) {
  
  if (labelSeries && labelSeries.mapImages.length) { // do nothing if labels do not yet exist
    
    // Update individual label if possible
    if (countryMapPolygon) {
      var label = countryMapPolygon.dummyData.label;
      if (label) {
        if (countryMapPolygon.dummyData.surfaceAreaPx >= visibilityThreshold) {
          label.show();
        } else {
          label.hide();
        }
    ...
    // Update all labels
      ...
      labelSeries.mapImages.each(function(label) {
        
        // Here we associate the rendered label to its relative country,
        // this way we can update the label as we loop through countries, as
        // opposed to looping through both.
        if ( ! label.dataItem.dataContext.countryMapPolygon.dummyData.label) {
          label.dataItem.dataContext.countryMapPolygon.dummyData.label = label; // now the references become circular
        }
        
        // Toggle label's Sprite State depending on country's surface area
        if (label.dataItem.dataContext.countryMapPolygon.dummyData.surfaceAreaPx >= visibilityThreshold) {
          label.show();
        } else {
          label.hide();
        }
...
// The whole map itself has a zoom event.
// 
// This event triggers repeatedly while zooming, if considering debouncing the
// event handler, there will be tradeoffs in performance, e.g. the zoom may feel
// more slick, but the show/hide of labels may feel delayed or off sync.
chart.events.on("zoomlevelchanged", function() {  
  
  var fn = undefined;
  
  // Update labels' visibility if they already exist
  if (labelSeries && labelSeries.mapImages.length) {
    fn = updateLabelVisibility;
  }
  
  updateAllSurfaceAreaPxData(fn);
...
// Once the countries' map polygons have loaded, we can piggy back off of their
// data to provide:
// 1. a reference to the country's map polygon (meta data), and
// 2. the name of the country itself (what we'll see on the screen)
polygonSeries.events.on("inited", function() {
  
  var labels = []; // will become the labelSeries' data
  
  polygonSeries.mapPolygons.each(function(mapPolygon) {
  
    updateSurfaceAreaPxData(mapPolygon);
    
    var center = countryCenterGeoPoints[mapPolygon.dataItem.dataContext.id]; // countryCenterGeoPoints is provided below
    if (center) {
      labels.push({
        "countryMapPolygon": mapPolygon, // circular reference
  ...
  // Create image series for labels
  // 
  // If we were to create this sooner, i.e. before/outside the countries' 
  // polygonSeries "inited" event, we cannot reasonably rely on the labelSeries'
  // "inited" event to determine when the labels themselves would be ready.
  // 
  // The "inited" event would trigger before the data is assigned, we'd then have
  // to rely on "validated" and similar events to run but on their second time.
  // (Their first time being when the series is ready, second being when the 
  // labels have been added.)
  labelSeries = chart.series.push(new am4maps.MapImageSeries());
  
  // Create label images in image series template so it gets replicated to all new images
  var labelSeriesTemplate = labelSeries.mapImages.template;
  var label = labelSeriesTemplate.createChild(am4core.Label);
  ...
  // Simple way of keeping the label's font size "small" as you zoom in/out.
  ...
  // We can have labels hidden by default by setting their Sprite State to hidden
  // at the template level.
  labelSeriesTemplate.setState("hidden");
  
  // Update labels' state as soon as all of them are rendered for the first time
  labelSeries.events.on("inited", function() {
    updateLabelVisibility();
  });
  
  // Even though the data assignment triggers the creation of label mapImages,
  // the rendered labels won't be immediately available on the next line of code,
  // this is why we rely on the "inited" event above.
  labelSeries.data = labels;
});

// Since we can't reliably calculate a country's visual center,
// we need a database for those.
var countryCenterGeoPoints = {
